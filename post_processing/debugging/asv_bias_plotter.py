...
def plot_bias(csv_path: Path, output: Path | None = None, show: bool = True) -> None:
    ...
    available = [key for key, col in resolved.items() if not df[col].isna().all()]
    if not available:
        raise ValueError(
            f"CSV '{csv_path}' contains bias columns, but every bias sample is NaN."
        )
    ...
    axes[1].grid(True)
    if any(k in available for k in accel_keys):
        axes[1].legend()
...
def main() -> None:
    csv_path = Path("microampere_ros2ws/src/microamp_fgo_rov_tracking/post_processing/estimated_data/scenario3/boat_estimated_20260420_112719.csv")
    output_path = Path("microampere_ros2ws/src/microamp_fgo_rov_tracking/post_processing/plots/scenario3/imu_bias_plot2.png")

    plot_bias(csv_path, output_path, show=True)
